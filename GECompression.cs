...
namespace DK64Viewer
{
  public class GECompression
  {
    ...
    public GECompression()
    {
      ...
      this.game = GECompression.DONKEYKONG64;
    ...
    public bool CompressGZipFile(string inputFile, string outputFile, bool byteFlipCompressed)
    {
      ...
      {
        uint count2 = count1;
        if (this.game != GECompression.BANJOTOOIE && count2 % 16U != 0U)
          count2 = (uint) ((int) count2 - (int) (count2 % 16U) + 16);
      ...
      if (this.IsFileExist(str3))
      {
        ...
        using (BinaryWriter binaryWriter = new BinaryWriter((Stream) File.Open(outputFile, FileMode.Create)))
        {
          ...
          {
            index1 -= 2U;
            ushort num = (ushort) Math.Ceiling((double) count1 / 16.0);
            buffer2[(int) index1] = (byte) ((int) num >> 8 & (int) byte.MaxValue);
            buffer2[(int) index1 + 1] = (byte) ((uint) num & (uint) byte.MaxValue);
          }
          else if (this.game == GECompression.CONKER)
          {
            index1 -= 4U;
    ...
    public byte[] OutputDecompressedBuffer(ref int fileSize, ref int compressedSize)
    {
      ...
      byte[] numArray = new byte[(int) this.maxByteSize];
      fileSize = 0;
      this.iterationCounter = 0U;
      while (this.iterationCounter <= 268435455U)
      {
        ...
        {
          case 0:
            if (!this.UncompressType0(numArray, ref fileSize))
            {
              fileSize = 0;
              return (byte[]) null;
            }
            break;
          case 1:
            if (!this.UncompressType1(numArray, ref fileSize))
            {
              fileSize = 0;
              return (byte[]) null;
            }
            break;
          case 2:
            if (!this.UncompressType2(numArray, ref fileSize))
            {
              fileSize = 0;
              return (byte[]) null;
            }
        ...
        {
          compressedSize = (int) this.bytesIndex;
          return ((IEnumerable<byte>) numArray).Take<byte>(fileSize).ToArray<byte>();
        }
      }
      fileSize = 0;
      return (byte[]) null;
